export const setEnLng = 'AppStateEnLng';
export const setThLng = 'AppStateThLng';
export const isSignIn = 'isSignIn';
export const SET_LOADING = 'SET_LOADING';
export const SET_SIGNINERROR = 'SET_SIGNINERROR';
export const SET_SNACKBAR_DISPLAY = 'SET_SNACKBAR_DISPLAY';
export const SET_SNACKBAR_DISMISS = 'SET_SNACKBAR_DISMISS';

export const SET_USER = 'SETUSERINFO';
export const CLEAR_USER = 'CLEARUSERINFO';

// Modal
export const AddAddressModal = 'AddAddressModal';
export const AddEmergencyContactModal = 'AddEmergencyContactModal';
export const UploadPictureModal = 'UploadPictureModal';

// Activity
export const SET_ACTIVITY = 'SET_ACTIVITY';
export const SET_UPCOMING_ACTIVITIES = 'SET_UPCOMING_ACTIVITIES';
export const SET_HISTORY_ACTIVITIES = 'SET_HISTORY_ACTIVITIES';
export const SET_FILTERED_ACTIVITIES = 'SET_FILTERED_ACTIVITIES';
export const SET_ACTIVITIES = 'SET_ACTIVITIES';

// Community
export const SET_FILTERED_USER_POST = 'SET_FILTERED_USER_POST';
export const SET_USER_POST_BY_ACTIVITY = 'SET_USER_POST_BY_ACTIVITY';
